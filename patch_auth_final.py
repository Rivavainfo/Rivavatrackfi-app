...
# Make precision replacements to avoid breaking syntax
...
content = re.sub(r'\s*tempName = account\.displayName.*?\n', '\n', content)
content = re.sub(r'\s*tempEmail = account\.email.*?\n', '\n', content)
content = re.sub(r'\s*tempPhotoUrl = account\.photoUrl\?\.toString\(\).*?\n', '\n', content)
